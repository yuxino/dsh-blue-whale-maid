...
`;
		...
		/** Balance bubble text from the host route payload. */
		function balanceText(payload, todayConsumed) {
			const infos = payload && Array.isArray(payload.balance_infos) ? payload.balance_infos : [];
			const info = infos[0];
			if (!info) return "💰 账户暂无余额信息";
			const currency = info.currency ?? "CNY";
			const lines = [`💰 余额 ${fmtMoney(Number(info.total_balance), currency)}`];
			const extra = [];
			...
			if (typeof todayConsumed === "number" && Number.isFinite(todayConsumed)) {
				lines.push(`今日约消费 ≈${fmtMoney(todayConsumed, currency)}`);
			}
			return lines.join("\n");
		}
		...
		function createPetEngine({ root, canvas, getSignal, show, openSession }) {
			...
			async function queryAccount() {
				...
				gesture("jump");
				emitHearts(6);
				let text = balanceText(balance, todayConsumed);
				const cost = fmtCost(sessionCost);
				if (cost !== null) text += `\n本会话已用 ${cost}`;
				show(text, 8000, { label: "刷新", fn: () => queryAccount() });
			}
			...
			function onPointerUp(ev) {
				...
				if (!dragStart.moved) {
					...
					if (lastClickAt !== null && now - lastClickAt < 350) {
						...
						gesture("wave");
						emitHearts(3);
						if (wasNapping) show("呼啊……醒啦！刚才梦到小鱼干山了~", 5000, { label: BALANCE_ACTION_LABEL, fn: () => queryAccount() });
						else show(pick(LINES.wave), 4000, { label: BALANCE_ACTION_LABEL, fn: () => queryAccount() });
						wasNapping = false;
					}
				} else {
