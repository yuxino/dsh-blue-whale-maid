...
function computeTodayConsumed(ctx, balance) {
	...
	// No history yet — we can't know today's opening balance, so there is no
	// meaningful estimate; record the current balance as the new baseline and
	// report null (the UI hides "今日约消费" rather than showing a fake 0).
	if (stored === null) {
		saveDayState(path, { date: today, opening: balance, last: balance });
		return null;
	}
	const opening = stored.date === today ? stored.opening : stored.last;
